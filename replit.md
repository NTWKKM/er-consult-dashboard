...
**Component Structure**:
- **Page Components**: 
  - Dashboard (`app/page.tsx`): Real-time dashboard with solid color theme. **Compact header**: Single-line title "ER-MNRH Consult Dashboard" with pending case counter and filter toggles (Surgery/Ortho/Both) on the same row for space efficiency. **Quick Navigation**: Minimal design collapsible dropdown showing all 8 departments (7 Surgery + 1 Ortho) as shortcut buttons with case counts; Surgery departments have coral border/hover, Ortho has green border/hover; clicking any department scrolls smoothly to that section. **Responsive layout**: Surgery section displays 1/2/3 columns (mobile/tablet/desktop). Ortho section displays 1/2 columns when viewing Ortho only; when viewing Both, Surgery takes 75% width (3 cols) and Ortho takes 25% width (1 col). Uses scalpel icon for Surgery (coral header) and bone icon for Ortho (green header). Light gray-blue card backgrounds with navy text. Performance optimized with query limit(30), useMemo for department filtering, and useRef for scroll targets. **Error handling**: Shows light-themed error screen with green reload button if Firebase connection fails.
