...
**November 10, 2025**:
- **Sound Notification System**: Added audio alerts when new consultation cases arrive with toggle control in dashboard header. Uses reusable AudioContext pattern for optimal performance. Sound preferences persisted in localStorage.
- **Dark Mode**: Implemented comprehensive night shift theme with toggle button in dashboard header. Dark mode affects all components including ConsultCard with proper contrast ratios. Theme preference persisted in localStorage for consistent user experience.
- **Bottom Navigation**: Created mobile-friendly fixed bottom navigation bar (hidden on desktop) with active route indicators using usePathname. Features quick access to Dashboard, Submit Consult, and Completed Cases pages with visual feedback for current location.
- **Responsive Layout Optimization**: Surgery section displays 1/2/3 columns (mobile/tablet/desktop); Ortho displays 1/2 columns when viewing alone, or 1 column when viewing Both (75% Surgery + 25% Ortho width split).
...
**Component Structure**:
- **Page Components**: 
  - Dashboard (`app/page.tsx`): Real-time dashboard with solid color theme and dark mode support. **Compact header**: Single-line title "ER-MNRH Consult Dashboard" with pending case counter and filter toggles (Surgery/Ortho/Both), sound notification toggle, and dark mode toggle all on the same row for space efficiency. **Sound Notifications**: Beep alert plays when new cases arrive (toggleable); uses reusable AudioContext for performance. **Dark Mode**: Night shift theme with localStorage persistence; affects all UI elements with proper contrast. **Quick Navigation**: Minimal design collapsible dropdown showing all 8 departments (7 Surgery + 1 Ortho) as shortcut buttons with case counts; Surgery departments have coral border/hover, Ortho has green border/hover; clicking any department scrolls smoothly to that section. **Responsive layout**: Surgery section displays 1/2/3 columns (mobile/tablet/desktop). Ortho section displays 1/2 columns when viewing Ortho only; when viewing Both, Surgery takes 75% width (3 cols) and Ortho takes 25% width (1 col). Uses scalpel icon for Surgery (coral header) and bone icon for Ortho (green header). Light gray-blue card backgrounds with navy text (or dark gray backgrounds in dark mode). Performance optimized with query limit(30), useMemo for department filtering, and useRef for scroll targets. **Error handling**: Shows light-themed error screen with green reload button if Firebase connection fails.
...
- **Shared Components**: 
  - `ConsultCard`: Dual-theme card with solid gray-blue background (light mode) or dark gray background (dark mode), patient icon, status badges, room location, and hover lift effects with glow. **Dark Mode Support**: All text elements dynamically adjust colors based on darkMode prop for proper contrast (gray-100/300/400 on dark backgrounds, navy #014167 on light). Regular cases display green accent border; urgent cases display coral border with pulsing glow effect, coral icon with lightning bolt, and "ด่วน" (urgent) badge. **Multi-department display**: Shows other consulted departments in underlined text below the main department. **Accept behavior**: "รับเคส" (Accept Case) button accepts the case for departments in the SAME CATEGORY only - clicking accept in any Surgery department (Gen Sx, Sx Trauma, Neuro Sx, Sx Vascular, Sx Plastic, Uro Sx, CVT) accepts ALL Surgery departments simultaneously; Ortho accepts only Ortho. Stores acceptedAt timestamp for all accepted departments in Firestore. **Close behavior**: "ปิดเคส" (Close Case) button closes only the specific department shown on the card, uses solid coral background. **Card sorting**: Urgent cases always appear first, sorted by creation time (oldest first) within urgent/normal groups.
  - `BottomNav`: Mobile-only fixed bottom navigation (hidden on desktop via lg:hidden) with active route highlighting. Uses usePathname hook to detect current page and applies visual feedback (colored backgrounds and icons). Features three main links: Dashboard (home icon), Submit Consult (plus icon with pulse dot), and Completed Cases (clipboard icon). Solid navy background with coral/green/peach accents matching main color palette.
- **Layout**: Compact sticky top navigation bar with solid coral logo, navy background, "จัดการเคส" (completed cases) link with gray-blue button, and prominent green submit button. Bottom navigation visible only on mobile devices (lg:hidden) with 80px padding on main content to prevent overlap.
